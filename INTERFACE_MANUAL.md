...
Pravá horní část okna je vymezená pro dvě části rozhraní. Panel **Statistiky, rychlost simulace** je zobrazený, pokud je daný experiment vytvořený. Panel **Počáteční nastavení** je zobrazený, pokud není zvolený experiment vytvořený.
...
* **`MAXAGE`** Ukazuje aktuální nejvyšší věk buněk (tj. věk nejstarší buňky). Na cedulku lze kliknout levým tlačítkem myši a označit tak nejstarší buňku.
...
* První řádek (červený) ukazuje informace o políčku, na němž je buňka, a to jeho hexagonové souřadnice tvaru `XX, XX, XX` (více v **Petriho miska**), energii obsaženou v políčku `energy: XXXX/1000`.
* Druhý řádek (zelený) ukazuje základní informace o buňce, a to její hexagonovou pozici `Hex pos: XX, XX, XX`, kolik má energie a maximální množství energie `energy: XXXX/XXXX`, jakou má velikost `size: XXXX`, jak je stará `age: XXXX`.
...
### Základní nastavení
Panel obsahuje 5 posuvníků **`TILE REFILL PERCENTAGE`**, **`PERCENTAGE OF REFILLED TILES`**, **`CELL ENERGY ABSORB PERCENTAGE`**, **`CELL ENERGY CONSUMPTION PERCENTAGE`**, **`CELL ENERGY TILE LOSS PERCENTAGE`**, které slouží k jednoduché úpravě experimentu, a to i za běhu simulace.
...
* **`PERCENTAGE OF REFILLED TILES`** Hodnota posuvníku udává, kolik procent políček se v každém cyklu vybere pro obnovení energie, tj. tomuto procentu se energie obnoví podle hodnoty **`TILE REFILL PERCENTAGE`** a ostatním se energie neobnoví.
...
* **`ENERGY EAT PERCENTAGE`** Hodnota v poli udává, kolik procent energie z cizí buňky získá buňka, která tuto cizí buňku pohltí.
